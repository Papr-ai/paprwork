...
### Issue 120: Taking Over the Browser Port Killed Paprwork Itself ✅ FIXED
**Added:** 2026-09-23
**Problem:** Paprwork quit seconds after every launch. The last lines before the graceful shutdown were `[PaprChromeSession] Port 9333 has a foreign Chrome profile … respawning Papr profile` and `[DirectChromeSpawn] Port 9333 is in use by another Chrome — stopping it`, followed immediately by `[Electron] App is quitting`. It recurred on every open because auto-continue resumed an interrupted turn that called a browser tool first thing.
**Root Cause:** Two defects, one configuration and one code, and the code one is the reason a config slip could kill the app.
1. **Electron was listening on the browser port.** `.env.local` still carried `PAPR_PLATFORM_EMBEDDED_CDP=1` / `PAPR_PLATFORM_CDP_PORT=9333`, added *temporarily* during the Enhancement 115 profiling session and never removed. Embedded CDP makes Electron serve DevTools on `PAPR_PLATFORM_CDP_PORT` — the same variable the gateway reads as "the port real Chrome is launched on". So the gateway probed 9333, found a CDP endpoint that was not Chrome with Papr's profile, and called it foreign.
2. **The kill signalled whatever it found.** `killChromeListeningOnPort` sent SIGTERM to every PID `lsof -ti tcp:PORT` returned — without `-sTCP:LISTEN`, which returns every process with *any* socket on the port, clients included — and never checked that a PID was Chrome, or that it was not the gateway or its Electron parent. Verified against real processes: with a listener in one process and a client in another, the old command returned both.
**The second victim, independent of the config:** the gateway probes `/json/version` immediately before deciding to kill, and Node's `fetch` keeps that connection alive. So even with a genuinely foreign Chrome on the port, the old kill could SIGTERM the gateway itself.
**Solution:** `lsof -nP -t -iTCP:<port> -sTCP:LISTEN` counts listeners only. `planCdpPortTakeover` classifies each holder as `own_process` (the gateway's `pid`/`ppid`), `chrome`, or `other`, and only a separate Chrome is ever signalled. Anything else **blocks the takeover before any signal is sent** — stopping the Chrome alone would not free the port — and throws a message naming the cause, which surfaces as a tool failure rather than a dead app. The SIGKILL escalation is limited to the PIDs chosen for SIGTERM. `looksLikeChromeCommandLine` excludes Electron, since Electron is Chromium and a bare `includes("Chrome")` matches it whenever a Chromium flag is on its command line.
**Files Changed:** `src/gateway/services/platforms/platformDirectChromeSpawn.ts`, `tests/platform-direct-chrome-spawn.test.ts` (+10 tests, three against real sockets)
**Verification:** gateway type-check 0 errors; 25 tests pass across the four platform-Chrome suites; lint clean. Four mutations caught — dropping `-sTCP:LISTEN`, the own-PID guard, the Electron exclusion, and restricting SIGKILL to the chosen PIDs. The Electron-exclusion mutation initially survived because the fixture's Electron command line never contained "Chrome"; the fixture, not the code, was the hole.
**Prevention:** A function that kills processes must positively identify each one, and must exclude its own process tree before anything else — a name like `killChromeListeningOnPort` is a promise the body has to keep. `lsof -i :port` is not "who serves this port". And a temporary diagnostic setting needs its removal tracked like any other change: this one was marked "remove both afterwards" in the file itself and still outlived the session by days.
**Related:** Enhancement 115 (the profiling session that added the flags), Issue 105 ("verify the fix is running"), Issue 84 (a native failure ending a process — here the process was ours, ended by our own code)
