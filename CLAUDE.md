...
3. **Historical rows are still inflated.** The fix stops new ones; recomputing the column is a separate migration decision.
...
**Related:** Issue 88 (established the inclusive convention and the `resolveStepContextTokens` detection reused here), Issue 85 (usage accumulation across streams — the same `promptTokens` this bills, and the source of the corrupted rows), Enhancement 87 (turn metrics, whose `cost` column this corrects)
