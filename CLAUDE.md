...
2. **`turn_peak_context_tokens` understates the billed prompt** — 181,281 recorded against 341,471 billed, because it counts neither the ~87K of tool schemas nor the system prompt.
...
### Issue 91: A Turn Is Recorded As Its Final Step ⏳ NOT FIXED
**Added:** 2026-09-11
...
**Why not fixed here:** the fix needs a fact the local data cannot supply — whether `totalUsage` in the installed `@ai-sdk/anthropic` sums cache fields across steps, or whether we must accumulate them ourselves. Guessing risks the mirror-image bug: accumulating a figure that is already summed would multiply costs by step count instead of dividing them. Issue 85's `addTurnUsage` is the natural place once that is settled, and it already distinguishes "absent" from "zero", which the cache fields need.
**Prevention:** A quantity that should grow with work done is worth checking against work done — one query grouping cost by step count would have caught this at any point in the last six weeks. And when a cross-check agrees suspiciously well, consider that two errors may be cancelling: Issue 90's "✅ exact" match was produced by one formula compared against itself, and this defect was hiding inside the same number.
**Related:** Issue 90 (the opposite-sign error that masked this), Issue 85 (fixed cross-stream accumulation and codified the false within-stream premise), Issue 88 (per-step context tokens, where the same cumulative-vs-per-request question was answered for `inputTokens`), Enhancement 87 (`turn_steps`, the instrument that made this visible)
