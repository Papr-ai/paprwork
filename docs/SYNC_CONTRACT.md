...
> each resource. Implementation details and phased rollout live in
> [`SYNC_ARCHITECTURE_V2.md`](./SYNC_ARCHITECTURE_V2.md). Publish/cache pipeline
> details live in [`CLOUD_SYNC_READY_PIPELINE.md`](./CLOUD_SYNC_READY_PIPELINE.md).

**Status:** Active (2026-08-06). Decisions below are binding for new sync work.
...
4. **Routine row sync is oplog-only.** Bulk table copy (bootstrap, snapshot fallback, full pull) is allowed only for explicit bootstrap/repair — never as a silent heuristic fallback.
...
- `apps/{id}/` (source, dist, backend bundle, linked-databases.json, revision marker)
- Dependent `Jobs/{id}/` folders
- `data/` when app links registry DBs (`databases.json`, migrations SQL — **not** `.db` files)
- `workspace/`, `data/` index files (instant sync)
...
|------|----------|-------------------------|
| App/job code → git | 30–45s | 5 min |
| `workspace/` / `data/` → git | 15s | 2 min |
| DB write → Turso | 30s (`TURSO_PUSH_DEBOUNCE_MS`) | **120s required** |
...
3. Desktop Turso hydration uses **sync-index poll** on heartbeat (`syncTursoFromSyncIndex`); `pendingCloudRuns` (git) is unchanged.
...
|--------|----------------------|---------------------------|
| **Upload now / `push_cloud_sync({ appId })`** | **Today:** Git and Turso run independently (`CloudSyncService.pushAppNow`). **Target (§12.1):** ordered pipeline — local migrate → Turso schema → Turso rows → git → verify → publish. Errors reported per layer. | Open browser tabs refreshed; scratch/unlinked DBs; other apps’ folders |
...
|---------------|----------------|
| **Git** | App code, `dist/`, migrations **SQL files**, `databases.json`, registry metadata | SQLite `.db` files, live row data |
| **Turso** | Row data, remote schema (via migration replay), CDC oplog | App TypeScript/UI |
...
2. Turso: replay migrations on remote (schema) — MUST complete before row push
3. Turso: push row deltas (oplog only)
4. Git: prepare (dist, bundle.json, revision marker) → commit → push
5. Verify: remote table set + migration ledger + spot row counts (§10)
6. Publish catalog reconcile + cache revision notify
```

**Rule:** Do not report **web-ready** until steps 2–5 pass for all linked sources. Git-only “synced” with Turso `schemaDrift` or `pending` = **overall not ready**.
...
| 5 | Post-push verify + convergence hash | **Done** — `postPushVerify.ts` |
