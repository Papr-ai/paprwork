...
1. **Documents** - `$PAPR_HOME/documents/{id}/meta.json` with `favorite: boolean`
2. **Apps** - `$PAPR_HOME/data/apps.json` with `favorite: boolean`
...
- `document` - Individual documents (`$PAPR_HOME/documents/{id}/`)
- `app` - Mini-apps (`$PAPR_HOME/apps/{id}/`)
...
2. useArtifacts.toggleFavorite() updates artifact metadata
   ↓
3. Saved to: $PAPR_HOME/documents/{id}/meta.json or $PAPR_HOME/data/apps.json
   ↓
...
### Storage Kept
- `$PAPR_HOME/documents/{id}/meta.json` - Document favorites ✅
- `$PAPR_HOME/data/apps.json` - App favorites ✅
