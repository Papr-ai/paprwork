...
Papr documents are Markdown files stored on disk at `$PAPR_HOME/documents/{docId}/content.md`. The UI auto-syncs changes via file watching.
...
```bash
# Append content
echo "\n## New Section\n\nContent here..." >> $PAPR_HOME/documents/{docId}/content.md

# Replace specific content
sed -i '' 's/old text/new text/g' $PAPR_HOME/documents/{docId}/content.md
```
