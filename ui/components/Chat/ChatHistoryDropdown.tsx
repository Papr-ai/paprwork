...
// Group chats by date
function groupChatsByDate(chats: ChatMetadata[]): Record<string, ChatMetadata[]> {
  const now = new Date();
  ...
  chats.forEach((chat) => {
    ...
    } else if (chatDay.getTime() > new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000).getTime()) {
...
export const ChatHistoryDropdown: React.FC<ChatHistoryDropdownProps> = ({ onClose }) => {
