...
function renderSequence(
  ...
  console.log('[MessageItem] Rendering sequence:', JSON.stringify(sequence, null, 2));
  ...
  if (hasTools) {
    ...
    const planCardMap = new Map<string, Parameters<typeof PlanCard>[0]["data"]>();
    ...
    sequence.forEach((item, index) => {
      ...
      if (item.type === 'text' && typeof item.data === 'string' && item.data.trim()) {
        ...
        if (index > lastToolIndex) {
          finalTextAfterAllTools = (finalTextAfterAllTools || '') + item.data.trim();
        } else {
        ...
        if ((toolName === 'create_plan' || toolName === 'update_plan') && toolCall.result) {
          const planData = parsePlanFromToolResult(toolName, typeof toolCall.result === 'string' ? toolCall.result : JSON.stringify(toolCall.result));
          if (planData) {
        ...
        exploringItems.push(
          <div key={`tool-${index}`} className="exploring-tool-item">
            ...
            {toolCall.status === 'success' && <span className="exploring-tool-success">✓</span>}
            {toolCall.status === 'error' && <span className="exploring-tool-error">✗</span>}
          </div>
        );
      }
    });
    ...
    if (exploringItems.length > 0) {
      elements.push(
        <div key="exploring" className="exploring-card">
          ...
          <div className="exploring-card-content" style={{ maxHeight: '200px', opacity: '1' }}>
            {exploringItems}
          </div>
        </div>
      );
...
export const MessageItem: React.FC<MessageItemProps> = ({ message }) => {
  ...
  return (
    <div className="message-item" data-testid={`message-item-${isUser ? "user" : "assistant"}`}>
      ...
      <div className="message-content">
        ...
        {hasSequence && !isUser ? (
          ...
          <>
            ...
            {!isUser && (() => {
              const planMap = new Map<string, Parameters<typeof PlanCard>[0]["data"]>();
            ...
            {content && (!message.toolCalls || message.toolCalls.length === 0) && (
