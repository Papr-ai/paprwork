...
export const useTabStore = create<TabState>()(
  ...
  createTab: (type, entityId, title, metadata = {}) => {
    ...
    if (
      ...
      console.log("[TabStore] No empty temp chat found, creating new chat tab");
    }
    ...
    console.log(`[TabStore.createTab] Type: ${type}, EntityId: ${entityId}, Title: ${title}`);
    
    set((state) => {
      const newTabs = [...state.tabs, newTab];
      console.log(`[TabStore.createTab] BEFORE set() - activeTabId: ${state.activeTabId}`);
      console.log(`[TabStore.createTab] BEFORE set() - tabs count: ${state.tabs.length}`);
      ...
      console.log(`[TabStore.createTab] RETURNING new state - activeTabId: ${newState.activeTabId}`);
      console.log(`[TabStore.createTab] RETURNING new state - tabs count: ${newState.tabs.length}`);
    ...
    console.log(`[TabStore.createTab] AFTER set() - activeTabId: ${currentState.activeTabId}`);
    console.log(`[TabStore.createTab] AFTER set() - tabs count: ${currentState.tabs.length}`);
    console.log(`[TabStore.createTab] AFTER set() - tabs:`, currentState.tabs.map(t => ({ id: t.id, entityId: t.entityId })));
  ...
  switchToTab: (tabId, skipHistory = false) => {
    ...
    if (!tab) {
      ...
      console.error(`[TabStore.switchToTab] Available tabs:`, get().tabs.map(t => t.id));
      return;
    }

    console.log(`[TabStore.switchToTab] Found tab, displayMode: ${tab.displayMode}`);
    ...
    if (isParent) {
      ...
      console.log(`[TabStore.switchToTab] Setting standalone tab as active: ${tabId}`);
      set({
        activeTabId: tabId,
        activeLeftTab: tabId,
        activeRightTab: null,
      ...
      console.log(`[TabStore.switchToTab] Active tab is now: ${get().activeTabId}`);
    }
  },
  ...
  closeTab: (tabId) => {
    ...
    if (idsToRemove.includes(state.activeTabId || "")) {
      const visibleTabs = state.tabs.filter((t) => t.displayMode !== "child");
      const tabIndex = visibleTabs.findIndex((t) => t.id === tabId);
      const nextTab =
  ...
  updateTabTitle: (tabId, title) => {
    ...
    if (!existingTab) {
      ...
      console.log("[TabStore] Available tabs:", get().tabs.map((t) => t.id));
      return;
    }
  ...
  goBack: () => {
    ...
    if (!get().getTab(targetTabId)) {
      ...
      const cleanedHistory = state.history.filter((id) => id !== targetTabId);
      set({
        history: cleanedHistory,
    ...
    set((state) => ({
      tabs: state.tabs.map((t) => {
        ...
        if (t.childTabIds.includes(oldTabId)) {
          return {
            ...t,
            childTabIds: t.childTabIds.map((id) => id === oldTabId ? newTabId : id),
          };
        }
        return t;
      }),
      ...
      activeRightTab: state.activeRightTab === oldTabId ? newTabId : state.activeRightTab,
      // Update history
  ...
  markTabAsRead: (tabId) => {
    set((state) => ({
      tabs: state.tabs.map((t) =>
        t.id === tabId
          ? { ...t, hasUnread: false, isStreaming: false }
          : t,
      ),
    }));
  },
