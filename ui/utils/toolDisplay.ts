...
export function getBashCommandDescription(command: string, isRunning = true): string {
  ...
  if (cmd.includes("cat >") && cmd.includes("<<")) {
    ...
    if (pathMatch) {
      ...
      if (filename) return isRunning ? `Updating ${filename}` : `Updated ${filename}`;
    }
  ...
  if (cmd.startsWith("cat ") && !cmd.includes(">")) {
    ...
    if (pathMatch) {
      const filename = getDisplayFilename(pathMatch[1].replace(/\\/g, ""));
      if (filename) return isRunning ? `Reading ${filename}` : `Read ${filename}`;
    }
  ...
  if (cmd.startsWith("grep")) {
    ...
    if (cmd.includes("documents/")) return isRunning ? "Searching documents" : "Searched documents";
  ...
  if (cmd.startsWith("ls")) {
    ...
    if (pathMatch) {
      ...
      if (dirname) return isRunning ? `Listing ${dirname}` : `Listed ${dirname}`;
    }
  ...
  if (cmd.includes("pip install") || cmd.includes("pip3 install")) {
    ...
    if (pkgMatch) return isRunning ? `Installing ${pkgMatch[1]}` : `Installed ${pkgMatch[1]}`;
    return isRunning ? "Installing Python packages" : "Installed Python packages";
  }
  ...
  if (cmd.includes("npm run") || (cmd.includes("yarn") && !cmd.includes("yarn add"))) {
  ...
  if (cmd.startsWith("git clone")) {
    ...
    if (urlMatch) return isRunning ? `Cloning ${urlMatch[1]}` : `Cloned ${urlMatch[1]}`;
    return isRunning ? "Cloning repository" : "Cloned repository";
  }

  if (cmd.startsWith("git pull")) return isRunning ? "Updating repository" : "Updated repository";
  if (cmd.startsWith("git commit")) return isRunning ? "Committing changes" : "Committed changes";
  if (cmd.startsWith("git push")) return isRunning ? "Pushing to remote" : "Pushed to remote";
  if (cmd.startsWith("git ")) return isRunning ? "Running git command" : "Ran git command";

  if (cmd.startsWith("mkdir")) {
    const pathMatch = cmd.match(/mkdir\s+(?:-p\s+)?([^\s]+)/);
    if (pathMatch) {
      const dirname = getDisplayFilename(pathMatch[1]);
      if (dirname) return isRunning ? `Creating ${dirname}` : `Created ${dirname}`;
    }
  ...
  if (cmd.startsWith("rm")) {
    ...
    if (pathMatch) {
      const filename = getDisplayFilename(pathMatch[1].replace(/\\/g, ""));
      if (filename) return isRunning ? `Deleting ${filename}` : `Deleted ${filename}`;
    }
  ...
  if (cmd.startsWith("cp ")) {
    ...
    if (pathMatch) {
      ...
      if (filename) return isRunning ? `Copying to ${filename}` : `Copied to ${filename}`;
    }
  ...
  if (cmd.startsWith("mv ")) {
    ...
    if (pathMatch) {
      const filename = getDisplayFilename(pathMatch[2].replace(/\\/g, ""));
      if (filename) return isRunning ? `Moving to ${filename}` : `Moved to ${filename}`;
    }
...
export const TOOL_DESCRIPTIONS: Record<string, { running: string; complete: string }> = {
  // Bash
  ...
  list_directory: { running: "Listing directory", complete: "Directory listed" },
  ...
  import_document: { running: "Importing document", complete: "Document imported" },
  // Apps
  ...
  link_app_data_source: { running: "Linking data source", complete: "Data source linked" },
  ...
  list_job_files: { running: "Listing job files", complete: "Job files listed" },
  ...
  // Sub-agents
  create_sub_agent: { running: "Creating sub-agent", complete: "Sub-agent created" },
  list_sub_agents: { running: "Listing sub-agents", complete: "Sub-agents listed" },
  delete_sub_agent: { running: "Deleting sub-agent", complete: "Sub-agent deleted" },
  delegate_task: { running: "Delegating task", complete: "Task delegated" },
  get_delegation_run: { running: "Checking delegation", complete: "Delegation loaded" },
  list_delegation_runs: { running: "Listing delegations", complete: "Delegations listed" },
  // Planning
  ...
  search_agent_memory: { running: "Searching memory", complete: "Memory loaded" },
  register_schema: { running: "Registering schema", complete: "Schema registered" },
  // Skills
  create_skill: { running: "Creating skill", complete: "Skill created" },
  read_skill: { running: "Reading skill", complete: "Skill loaded" },
  // Browser
  browser_navigate: { running: "Navigating browser", complete: "Browser navigated" },
  ...
  browser_console_logs: { running: "Reading console logs", complete: "Console logs read" },
  browser_network_logs: { running: "Reading network logs", complete: "Network logs read" },
  browser_test_script: { running: "Running browser test", complete: "Browser test done" },
  // Webview
  webview_launch_app: { running: "Launching app preview", complete: "App preview ready" },
  webview_snapshot: { running: "Reading app preview", complete: "App preview read" },
  webview_execute: { running: "Running preview script", complete: "Preview script done" },
  webview_get_console: { running: "Reading preview console", complete: "Preview console read" },
  webview_get_network: { running: "Reading preview network", complete: "Preview network read" },
